...
<?php
    ...
    if (isset ($_POST['Simpan'])){
		...
		if($_POST['nip'] != '') { //kalau nipnya ngga kosong
			...
			$sql_nip = mysqli_query($koneksi,"SELECT CONCAT(DATE_FORMAT(CURDATE(),'%Y'),DATE_FORMAT(CURDATE(),'%m'),DATE_FORMAT(CURDATE(),'%d'),(COUNT(*)+1))  as NIP FROM data_pegawai");
		...
		if(!empty($sumber)){
        $sql_simpan = "INSERT INTO data_pegawai (nik, nip, nama_lengkap, tempat_lahir, tanggal_lahir, jenis_kelamin, agama, no_ktp, alamat,kode_pos, nomor_telepon, status_pernikahan, kedudukan_hukum, jenis_pegawai, satker, unit_kerja, pangkat_terakhir,jabatan_terakhir, pendidikan_terakhir, eselon, foto) VALUES (
			'".$_POST['nik']."',
			'".$newNip."',
			'".$_POST['nama_lengkap']."',
			'".$_POST['tempat_lahir']."',
