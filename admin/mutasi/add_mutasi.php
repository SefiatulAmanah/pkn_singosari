...
$pindah = move_uploaded_file($sumber, $target.$file_mutasi);
	...
	if($_POST['nik'] != '') { //kalau nipnya ngga kosong
		...
		$sql_nik = mysqli_query($koneksi,"SELECT CONCAT(DATE_FORMAT(CURDATE(),'%Y'),DATE_FORMAT(CURDATE(),'%m'),DATE_FORMAT(CURDATE(),'%d'),(COUNT(*)+1))  as NIP FROM data_pegawai");
	...
	if(!empty($sumber)){
        $sql_simpan = "INSERT INTO riwayat_mutasi_cpns_ke_pns (nik, nama, tmt_cpns, nomor_sk_cpns, tanggal_sk_cpns, nomor_bkn_pengangkatan_cpns, tmt_pns, nomor_sk_pns, tanggal_sk_pns, nomor_sttpl_diklat_pra_jabatan, tanggal_sttpl_diklat_pra_jabatan, nomor_surat_keterangan_sehat, tanggal_surat_keterangan_sehat, 
		nomor_skck, tanggal_skck, jenis_pengadaan_pegawai, mutasi ) VALUES (
			'".$_POST['nik']."',
			'".$_POST['nama']."',
			'".$_POST['tmt_cpns']."',
			'".$_POST['nomor_sk_cpns']."',
