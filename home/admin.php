...
<?php
	...
	$sql = $koneksi->query("SELECT count(Nik) as lokal from data_pegawai");
...
?>
 ...
 <?php
	$sql = $koneksi->query("SELECT count(nip) as putra from data_pegawai where LENGTH(nip) >= 12");
	while ($data= $sql->fetch_assoc()) {
		$putra=$data['putra'];
	}
?> 
...
<?php
	$sql = $koneksi->query("SELECT count(nip) as putri from data_pegawai where LENGTH(nip) < 12");
	while ($data= $sql->fetch_assoc()) {
